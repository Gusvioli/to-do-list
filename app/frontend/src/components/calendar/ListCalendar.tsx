...
function ListCalendar(): JSX.Element {
  ...
  const contlengthContents = (dataLoop: string): string => {
    if (getdataContents) {
      return getdataContents.filter((element: any) => element.date === dataLoop).length > 0
        ? getdataContents.filter((element: any) => element.date === dataLoop).length : ''
    } else {
      return ''
    }
  }

  const searshDate = () => search ? searchControl(search, getdataContents) : [];

  return (
    <>
      <div className="lists-calendar-div-0">
          {diasMeses().map((element: {
            dias: number;
              ...
              <div className="lists-calendar-div-div-1" >
                {new Array(element.dias).fill('').map((_, index) =>
                  (
                    ...
                    style={
                      searshDate()
                        .find((elementdate: any) => elementdate.date === getDateToSend(element, index))
                        ? {
                          border: '5px solid green',
                          opacity: '1',
                          borderRadius: '15px',
                          boxShadow: '0px 0px 10px 0px rgba(145, 170, 170, 0.75)'
                        }
