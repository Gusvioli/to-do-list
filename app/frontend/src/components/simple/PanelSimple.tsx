...
function PanelSimple({
  ...
  const hendleEdit = async (e: any) => {
    ...
    setEditrTrue(true)
    const emojiReturn = emojis.filter(
      (emoji: any) => emoji.name === emojiName.name,
    )
  ...
  const hendleDelete = async (e: any) => {
    ...
    await requestDelete('/deleteContents', {
      id,
      date,
      idUser,
    })
  ...
  const hendlestatus = async (e: any) => {
    ...
    await requestUpdate('/contentsUpdate', {
      id: Number(idSplit[1]),
      idUser: getIdUser,
      date,
      status: idSplit[0],
